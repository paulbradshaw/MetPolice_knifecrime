...
def scrapetable(root):
  ...
  uniqueid = 0
  #Grab any bits of 'root' (passed into 'scrapetable' as a parameter above) that have the tag <table> containing summary=" and 'Knife' somewhere in that, then the contents of <tr>. Put the results in variable called 'rows'
  rows = root.xpath(".//table[contains(@summary, 'Knife')]//tr")
  #That will be a list, so we start a for loop to go through each item, calling it
  'row'
  for row in rows:
    #show us the text content of that item
    print row.text_content()
    #now grab the contents of all <td><a ... > tags within that 'row' object, and put it in
    the variable 'report'
